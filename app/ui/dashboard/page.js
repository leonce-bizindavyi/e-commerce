...
function Dashboard() {
  ...
  useEffect(() => {
    if (socket) {
      socket.emit("admin", {connect: true})
      socket.on("admin_all_users", (infos) => {
        setUsers(infos)
      })
      return () => {
            ...
            <table className='min-w-full'>
              ...
              <tbody className='bg-white'>
                {users.map((user) => {
                  return (
                    <tr key={user.id}>
                      <td className='px-4 whitespace-no-wrap border-b border-gray-200'>
                        <div className='flex items-center space-x-7'>
                          <div className='flex-shrink-0 w-10 h-10'>
                            <div className='w-10 h-10 rounded-full bg-blue-500 uppercase flex items-center justify-center text-white text-4xl p-7'> 
                            {user.username[0]}
                            </div>
                          </div>
                          <div className='ml-4'>
